...
// Sluit menu bij klik op een menu-link (voor mobiel)
...
function setupColorDemo() {
  ...
  const colorExamples = {
    hex: {
      value: '#ff6b35',
      desc: 'Hexadecimaal - meest gebruikt formaat',
      color: '#ff6b35',
    },
    rgb: {
      value: 'rgb(29, 211, 176)',
      desc: 'Red Green Blue waarden (0-255)',
      color: 'rgb(29, 211, 176)',
    },
    hsl: {
      value: 'hsl(240, 100%, 70%)',
      desc: 'Hue Saturation Lightness - intuïtief',
      color: 'hsl(240, 100%, 70%)',
    },
    rgba: {
      value: 'rgba(123, 92, 255, 0.8)',
      desc: 'RGB met transparantie (alpha 0-1)',
...
function setupAnimationDemo() {
  ...
  if (durationSelect)
    durationSelect.addEventListener('change', updateTransitionDemo);
  if (easingSelect)
    easingSelect.addEventListener('change', updateTransitionDemo);
...
function setupFlexboxDemo() {
  ...
  [directionSelect, justifySelect, alignSelect, wrapSelect, gapInput].forEach(
    (control) => {
